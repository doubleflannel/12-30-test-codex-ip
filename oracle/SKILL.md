...
## Main use case (browser, GPT‑5.2)

Default workflow here: `--engine browser` with GPT‑5.2 in ChatGPT. This is the “human in the loop” path: it can take ~10 minutes to ~1 hour; expect a stored session you can reattach to.

Recommended defaults:
- Engine: browser (`--engine browser`)
- Model: GPT‑5.2 (either `--model gpt-5.2` or a ChatGPT picker label like `--model "5.2"`)
...
3. Run in browser mode for the usual GPT‑5.2 ChatGPT workflow; use API only when you explicitly want it.
...
- Preview (no tokens):
  - `npx -y oracle --dry-run summary -p "<task>" --file "src/**" --file "!**/*.test.*"`
  - `npx -y oracle --dry-run full -p "<task>" --file "src/**"`

- Token/cost sanity:
  - `npx -y oracle --dry-run summary --files-report -p "<task>" --file "src/**"`

- Browser run (main path; long-running is normal):
  - `npx -y oracle --engine browser --model gpt-5.2 -p "<task>" --file "src/**"`

- Manual paste fallback (assemble bundle, copy to clipboard):
  - `npx -y oracle --render --copy -p "<task>" --file "src/**"`
...
- If you need hidden/advanced knobs: `npx -y oracle --help --verbose`.
...
- Runs may detach or take a long time (browser + GPT‑5.2 often does). If the CLI times out: don’t re-run; reattach.
